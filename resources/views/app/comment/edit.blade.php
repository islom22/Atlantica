...
@section('content')
    ...
    <h1 class="text-uppercase mt-5">Comments</h1>

    <div class="headcrumbs d-flex mb-3">
        <a href="{{ route('comments.index') }}" class="d-flex text-uppercase me-2" style="opacity:25%">Comments</a> >> <a
                ...
                <div class="d-flex flex-wrap previewFiles" id="images">
                    ...
                    @if (!($comment->img == null))
                        ...
                        <div id="full_image" class="d-flex align-items-center justify-content-center me-2"
                            ...
                            <button class="delete  btn btn-danger btn-delete rmFile" data-id="{{ $comment->id }}"
                    ...
                    @if ($comment->img == null)
                        <div id="empty_image">
                            ...
                            {{-- @csrf --}}
                            <input type="hidden" name="id" value="{{ $comment->id }}">
                            <input name="img" id="add_img" type="file" data-id="{{ $comment->id }}"
                                        ...
                                        <div class="tab-pane fade" id="{{ $language->lang }}" role="tabpanel"
                                            ...
                                            <div class="row mb-2">
                                                ...
                                                <div class="row mb-2">
                                                    ...
                                                    @if (isset($comment->title[$language->small]))
                                                        <input type="text" class="form-control"
                                                            name="title[{{ $language->small }}]" placeholder="title"
                                                            value="{{ old('title.' . $language->small) ? old('title.' . $language->small) : $comment->title[$language->small] }}">
                                                    @else
                                                        <input type="text" class="form-control"
                                                            name="title[{{ $language->small }}]" placeholder="title"
                                                ...
                                                <div class="row mb-2">
                                                    ...
                                                    @if (isset($comment->desc[$language->small]))
                                                        <textarea class="form-control ckeditor" placeholder="Enter your description..." rows="4"
                                                            name="desc[{{ $language->small }}]">{{ old('desc.' . $language->small, $comment->desc[$language->small]) }}</textarea>
                                                    @else
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-delete', function() {
                ...
                if (id) {
                    ...
                    $.ajax({
                        url: "/admin/delete-image-comment",
                        type: 'POST',
                        dataType: "JSON",
                        data: {
                            "id": id,
                        ...
                        success: function() {
                            $("#images").load('http://localhost:8000/admin/comments/' + id +
                                '/edit #images');
            ...
            $(document).on('change', '.btn-update', function() {
                ...
                if (id) {
                    ...
                    $.ajax({
                        url: "/admin/upload-comment-image",
                        type: 'POST',
                        dataType: "JSON",
                        data: formData,
                        contentType: false,
